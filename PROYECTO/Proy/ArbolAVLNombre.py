...
class ArbolAVLNombre:
    ...
    def buscar(self, nombre):
        """
        Busca un nombre en el árbol y retorna True si lo encuentra, False si no lo encuentra.
        """
        nodo_actual = self.raiz
        while nodo_actual is not None:
            if nombre == nodo_actual.dato:
                return True
            elif nombre < nodo_actual.dato:
    ...
    def imprimir_en_orden(self, datos_egresados):
        self.__imprimir_en_orden_recursivo(self.__raiz, datos_egresados)

    def __imprimir_en_orden_recursivo(self, nodo, datos_egresados):
        if nodo is not None:
            self.__imprimir_en_orden_recursivo(nodo.izq, datos_egresados)
            egresado = next((egresado for egresado in datos_egresados if egresado[0] == nodo.dato), None)
            if egresado:
                nombre, profesion, promedio = egresado
                print(f"{nombre} -- {profesion} -- {promedio}")
            self.__imprimir_en_orden_recursivo(nodo.der, datos_egresados)