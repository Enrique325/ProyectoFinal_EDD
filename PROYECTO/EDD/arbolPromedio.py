...
class ArbolAVL:
    ...
    def buscar_por_calificacion(self, profesion):
        nodos_promedio = set()

        def buscar_calificacion_recursivo(nodo):
            if nodo is not None:
                if nodo.dato == profesion:
                    nodos_promedio.add(nodo.indice)
                    for dato, indice in nodo.datosiguales:
                        nodos_promedio.add(indice)
                buscar_calificacion_recursivo(nodo.izq)
                buscar_calificacion_recursivo(nodo.der)

        buscar_calificacion_recursivo(self.raiz)
        return nodos_promedio
...
if __name__ == '__main__':
    ...
    arbol.inorden()
    print(arbol.buscar_por_calificacion(93))
    print(arbol.listar_ascendente())
    print(arbol.listar_descendente())